...
// loadMergeFiles 加载所有合并文件，并将数据复制到原始数据目录。
// 如果没有合并文件，或者合并操作未完成，则返回为零。
...
func (db *DB) loadIndexFromHintFile() error {
	...
	// todo这个NewReader是干嘛的？作用是什么，如果有多个segment,这个read会读取所有吗
	...
	for {
		...
		//todo reader.next获取数据，然后计算出它的偏移量并存到map里
